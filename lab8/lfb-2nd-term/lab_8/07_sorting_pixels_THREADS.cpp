...
void partlySort(int part, int psize){
    for(uint32_t i = part * psize; i < psize * (part+1); i++)
        sort(pict[i]);
}
...
int main(int argc, char* argv[])
{
    ...
    int size_on_one_thread = height/N;
    std::vector<std::thread> theads;
    for(int i=0; i<N; i++){
        theads.push_back(std::thread(partlySort, i, size_on_one_thread));
    }
    for(int i=0; i<N; i++){
        theads[i].join();
    }
